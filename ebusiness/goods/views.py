...
#用户登出
def logout(request):
    response = HttpResponseRedirect('/index/') # 登录成功跳转查看拍卖品信息
...
def add_chart(request,good_id,sign):
    ...
    if username=="":
        ...
        #参与当前拍卖，参数为拍卖品编号，默认报价为1，有效时间为一年
...
def update_chart(request,good_id):
    ...
    if username=="":
        ...
        if int(count)<=0:
            ...
            my_chart_list = util.add_chart(request)
            #返回错误信息
            return render(request, "view_chart.html", {"user": username, "goodss": my_chart_list,"error":"报价不能为负"})
        else:
            #否则修改拍卖品报价
            response = HttpResponseRedirect('/view_chart/')
...
def order_status(request):
	models.Order.objects.filter('status').delete()
	# 首先删除Order表中原有Status
	max_order=Order.objects.values('goods').annotate(max_price=max('count'))
    # 在Order表中按照good_id分组，获取每组的最大报价
	order_all=Order.objects.all()
	for key in max_order:
		order_win=Order.objects.filter( count=key.max_price)
		order_id=order_win.order
		# 获得每个拍卖品最大报价的订单号
		for key2 in order_all:
			# 在所有订单中遍历
			if datetime.datetime.now()<key2.deadline:
				key2.status=0
				#若时间在截止时间之间，返回状态为0，“正在进行”
			elif order_id==key2.order and datetime.datetime.now()>key2.deadline:
				key2.status=1
				#若已达截止时间，且订单号为最大报价订单订单号，返回状态为1，“竞拍成功”
			else :
				key2.status=2
				# 其他情况下，返回状态为2，“竞拍失败”
		key2.save()
    ...
    if username=="":
        ...
        if address_id=="":
            ...
            for key in cookie_list:
                ...
                good = get_object_or_404(Goods, id=key)
                order.deadline = good.time
                #保存单个订单信息
                order.save()
...
def view_order(request,orders_id):
    ...
    if username=="":
        ...
        for key in order_filter:
            ...
            prices = order_object.count + prices
